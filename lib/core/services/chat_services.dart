...
class ChatService {
  ...
  Future<void> sendMessage({
    ...
    required String text,
    String? imageUrl,
    String? voiceUrl, // Added for voice messages
  }) async {
    try {
      final user = _auth.currentUser;
      if (user == null) throw Exception('User not authenticated');
      ...
      await _firestore
        ...
        'senderName': senderName,
        'imageUrl': imageUrl,
        'voiceUrl': voiceUrl, // Added for voice messages
  ...
  Future<String> uploadVoiceMessage(String groupId, String voicePath) async {
    try {
      ...
      final ref = _storage.ref().child('group_chats/$groupId/voice/$fileName');
      await ref.putFile(File(voicePath));
      return await ref.getDownloadURL();
    } catch (e) {
        ...
        .map((snapshot) {
          ...
          return snapshot.docs.map((doc) {
            ...
            return {
              ...
              'timestamp': data['timestamp'] ?? Timestamp.now(),
              'imageUrl': data['imageUrl'],
              'voiceUrl': data['voiceUrl'], // Added for voice messages
            };
          }).toList();
        });
  }
