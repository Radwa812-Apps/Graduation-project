...
class ChatService {
  ...
  ChatService({
    ...
    _cloudinaryService =
        cloudinary ??
        CloudinaryService(auth: _auth, encryptionKey: encryptionKey);
  ...
  Future<bool> isChatMuted(String recipientId) async {
    try {
      ...
      print(
        'isChatMuted: Chat with $recipientId is ${isMuted ? 'muted' : 'not muted'} for user $userId',
      );
      return isMuted;
    } catch (e) {
  ...
  Future<void> sendPrivateMessage({
    ...
    try {
      ...
      if (recipientToken != null && recipientToken.isNotEmpty && !isMuted) {
        ...
        print(
          'Notification not sent to $recipientId. ' +
              'Muted: $isMuted, ' +
        ...
        for (final chatDoc in recentChats.docs) {
          ...
          try {
            ...
            if (properName.isNotEmpty &&
                properName != chatData['recipientName']) {
              print(
                'Updating name for ${chatData['recipientName']} to $properName',
              );
...
''');
  ...
  Future<bool> isGroupMuted(String groupId) async {
    try {
      ...
      final isMuted = doc.exists && doc.data()?['muted'] == true;
      print(
        'isGroupMuted: Group $groupId is ${isMuted ? 'muted' : 'not muted'} for user $userId',
      );
      return isMuted;
    } catch (e) {
    ...
    try {
      ...
      if (members.isNotEmpty) {
        ...
        for (final userDoc in usersSnapshot.docs) {
          ...
          final isMuted = await isGroupMuted(
            groupId,
          ); // Check mute status for each recipient

          if (token != null && token.isNotEmpty && !isMuted) {
            final notificationTitle =
                'New message in ${groupDoc.data()?['name'] ?? 'group'}';
            String notificationBody;
            ...
            print(
              'Notification sent to $recipientId for message in group $groupId',
            );
          } else {
            print(
              'Notification not sent to $recipientId (muted: $isMuted, token: ${token != null})',
            );
          }
        }
      }
